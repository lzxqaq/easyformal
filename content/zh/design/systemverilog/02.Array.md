...
**Sr No.** | **数组类型**         | **描述**                                                                   | 
|---|------------------------------------------ | -------------------------------------------------------------------------------------|
1|[合并数组(Packed Array)](/02.Array#1packed-arrays) | 先声明数组名称，后声明数组维度 |       
2|[非合并数组(Unpacked Array)](./02.Array#2unpacked-arrays) | 先声明数组维度，后声明数组名称 | 
3|[Dynamic Array](./02.Array#3dynamic-arrays) | 内存将在运行时分配 |
4|[Associative Array](./02.Array#4associative-arrays) | Memories allocated only when it is used and any index type is used for indexing the array |
5|[Queue](./02.Array#5queue) | It is similar to fifo and we can add and remove elements from queue at run time |
6 | [Array Manipulation Methods](./02.Array#6array-manipulation-method) | It consist of array locator method, Array ordering method and Array Reduction method  |
