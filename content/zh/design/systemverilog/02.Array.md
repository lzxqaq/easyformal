...
3|[动态数组(Dynamic Array)](./02.Array#3dynamic-arrays) | 内存将在运行时分配 |
4|[关联数组(Associative Array)](./02.Array#4associative-arrays) | Memories allocated only when it is used and any index type is used for indexing the array |
5|[队列(Queue)](./02.Array#5queue) | It is similar to fifo and we can add and remove elements from queue at run time |
6 | [数组操作函数](./02.Array#6array-manipulation-method) | 它由数组定位函数、数组排序函数和数组缩减函数组成  |
